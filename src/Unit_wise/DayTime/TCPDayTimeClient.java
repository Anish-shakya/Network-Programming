...
public class TCPDayTimeClient {
    ...
    // Set the socket's timeout to 15 seconds (in milliseconds)
        InputStream in = socket.getInputStream(); 
    // Get an InputStream object from the socket
        StringBuilder time = new StringBuilder(); 
    // Create a new StringBuilder object to store the incoming data
        InputStreamReader reader = new InputStreamReader(in, "ASCII"); 
    // Create a new InputStreamReader object to read the data as ASCII characters
      for (int c = reader.read(); c != -1; c = reader.read()) { 
    // Start a for loop that reads characters from
